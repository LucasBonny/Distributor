    ...
    create table tb_payment (
        payment_method integer check (payment_method between 0 and 3),
        payment_status integer check (payment_status between 0 and 2),
        id bigint generated by default as identity,
    ...
    create table tb_product (
        ...
        bar_code bigint generated by default as identity,
        img_url varchar(255),
        name varchar(255),
    ...
    create table tb_sale (
        sale_status integer check (sale_status between 0 and 2),
        employee_id bigint,
        id bigint generated by default as identity,
    ...
    create table tb_supplier_product (
        price float(53) not null,
        quantity integer not null,
        date_delivery timestamp(6) with time zone,
        tb_product bigint not null,
        tb_supplier uuid not null,
        primary key (tb_product, tb_supplier)
    );

    alter table if exists tb_employee_role 
       add constraint FKpivoptm0s5gsnqtel1xc5tjwg 
       foreign key (role_id) 
       references tb_role;

    alter table if exists tb_employee_role 
       add constraint FK5a1thqkyfisgd225gsrauel61 
       foreign key (employee_id) 
       references tb_employee;

    alter table if exists tb_payment 
       add constraint FKhl0nnsn2x5cqc4avi989dfh8r 
       foreign key (sale_id) 
       references tb_sale;

    alter table if exists tb_sale 
       add constraint FK20unxq4gx5kiemos3y4yrv285 
       foreign key (employee_id) 
       references tb_employee;

    alter table if exists tb_sale_item 
       add constraint FKty6njf75edhxfqch4el0djkq 
       foreign key (product_id) 
       references tb_product;

    alter table if exists tb_sale_item 
       add constraint FKm127ckuaearcl3vyfxnb8e2i6 
       foreign key (sale_id) 
       references tb_sale;

    alter table if exists tb_supplier_product 
       add constraint FK4e2vomrfljvpuk3hwm575wlpy 
       foreign key (tb_product) 
       references tb_product;

    alter table if exists tb_supplier_product 
       add constraint FKfesgaase91p676g7knmgrln6j 
       foreign key (tb_supplier) 
